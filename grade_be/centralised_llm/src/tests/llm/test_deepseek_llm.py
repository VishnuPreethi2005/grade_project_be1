...
from src.llms.Deepseek_llm import DeepSeekLLM
from src.llm_base import GenerateRequest, GenerateResponse


class TestDeepSeekLLM:
    """Test suite for DeepSeekLLM class."""

    def test_init(self, monkeypatch):
        """Test DeepSeekLLM initialization with monkeypatch for env vars."""
        ...
        with patch('src.utils.config.get_env', return_value=None):
            ...
            assert llm.model == "deepseek-ai/DeepSeek-V3"
    ...
    async def test_generate_success(self, mock_together_generate, monkeypatch):
        ...
        expected_response = GenerateResponse(
            response="Test response from DeepSeek",
            prompt_tokens=15,
        ...
        result = await deepseek_llm.generate(request)
        ...
        assert kwargs['model'] == "deepseek-ai/DeepSeek-V3"
    ...
    @pytest.mark.asyncio
    @patch('src.llms.Deepseek_llm.together_generate_and_log')
    @patch('src.llms.Deepseek_llm.handle_together_api_error')
    ...
    @pytest.mark.asyncio
    @patch('src.llms.Deepseek_llm.together_batch_generate_and_log')
    async def test_batch_generate_success(self, mock_batch_generate, monkeypatch):
        ...
        expected_responses = [
            GenerateResponse(response="Response 1", prompt_tokens=10, completion_tokens=15, total_tokens=25, cost=0.001, model="deepseek-ai/DeepSeek-V3"),
            GenerateResponse(response="Response 2", prompt_tokens=12, completion_tokens=18, total_tokens=30, cost=0.002, model="deepseek-ai/DeepSeek-V3")
        ]
        mock_batch_generate.return_value = expected_responses

        requests = [
            GenerateRequest(model="deepseek-ai/DeepSeek-V3", prompt="Prompt 1", parameters={"temperature": 0.5}),
            GenerateRequest(model="deepseek-ai/DeepSeek-V3", prompt="Prompt 2", parameters={"temperature": 0.5})
        ]

        result = await deepseek_llm.batch_generate(requests)
    ...
    async def test_batch_generate_empty_requests(self, mock_batch_generate, monkeypatch):
        ...
        result = await deepseek_llm.batch_generate([])
    ...
    async def test_batch_generate_exception_handling(self, mock_handle_error, mock_batch_generate, monkeypatch):
        ...
        requests = [
            GenerateRequest(model="deepseek-ai/DeepSeek-V3", prompt="Prompt 1", parameters={}),
            GenerateRequest(model="deepseek-ai/DeepSeek-V3", prompt="Prompt 2", parameters={})
        ]
        ...
        for response in result:
            ...
            assert response.model == "deepseek-ai/DeepSeek-V3"
    ...
    def test_init_with_env_api_key(self, monkeypatch):
        ...
        with patch('src.utils.config.get_env', return_value="env-api-key"):
            llm = DeepSeekLLM(api_key="fallback-key", model="deepseek-ai/DeepSeek-V3")
