...
class AZUREOpenAILLM(CallbackCapableLLM, ErrorHandlingLLM, BaseLLM):
    def __init__(self, api_key: str, model: str, temperature: float = 0.7, 
                 max_tokens: int = 512, top_p: float = 1.0, n: int = 1, 
                 stop: Optional[Union[str, List[str]]] = None, 
                 frequency_penalty: float = 0.0, presence_penalty: float = 0.0):
        """
        Initialize AZUREOpenAILLM with all ChatCompletion API parameters.
        ...
        env_key = get_env('AZURE_OPENAI_SUBSCRIPTION_KEY')
    ...
    def _configure_llm(self):
        ...
        if "azure" in self.model.lower() or len(self.api_key) > 40:
    ...
    def handle_error(self, error: Exception, request: GenerateRequest) -> GenerateResponse:
        ...
        if "401" in error_str:
            ...
            error_message = "You exceeded your current quota: Please check your plan and billing details. Buy more credits or learn how to increase your limits."
    ...
    #     with mlflow.start_run(nested=True,run_name=f"Azure_OpenAI_{self.model}"):
    #         mlflow.log_params({
    #             "model": self.model,
    #             "temperature": self.temperature,
    #             "max_tokens": self.max_tokens,
