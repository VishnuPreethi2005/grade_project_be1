...
class MetaLlamaLLM(BaseLLM):
    def __init__(self, api_key: str, model: str = "meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo", 
                 ...
                 n: int = 1):
        """
        Initialize MetaLlamaLLM with all Meta Llama API parameters.
        
        Args:
            api_key: Together API key for Meta Llama models
            model: Model name (default: meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo)
            temperature: Randomness in response (0.0-1.0, default: 0.7)
            max_tokens: Maximum tokens to generate (default: 512)
            stop: Stop sequences (default: ["</s>", "<|eot_id|>"])
        ...
        self.stop = stop if stop is not None else ["</s>", "<|eot_id|>"]
        self.top_p = top_p
        self.top_k = top_k
    ...
    async def generate(self, request: GenerateRequest) -> GenerateResponse:
        try:
            ...
            logger.error(f"Meta Llama generation failed: {e}")
    ...
    async def batch_generate(self, requests: List[GenerateRequest]) -> List[GenerateResponse]:
        try:
            ...
            logger.error(f"Meta Llama batch generation failed: {e}")
            error_message = handle_together_api_error(e, self.model)
            return [GenerateResponse(
                response="",
                prompt_tokens=0,
